...
@globalActor actor ImageDatabase {
  ...
  private var inDiskContinuation: AsyncStream<Int>.Continuation?
  private var inDiskAccessCounter = 0 {
    didSet { inDiskContinuation?.yield(inDiskAccessCounter) }
  }
  
  deinit {
    inDiskContinuation?.finish()
  }
  ...
  func setUp() async throws {
    ...
    let accessStream = AsyncStream<Int> { continuation in
      inDiskContinuation = continuation
    }
